/**
 * GitHub API client for RRROCA Cloud Run chatbot.
 * Ported from api/shared/github.js
 */
...
export function createHttpError(status, message, logMessage) {
...
export function sanitizeLog(value, maxLength = 200) {
...
function mapGithubError(responseStatus) {
  ...
  if (responseStatus === 409) return createHttpError(409, 'Conflict — the file was modified. Please try again.');
  if (responseStatus === 422) return createHttpError(400, 'Invalid request to GitHub.');
  if (responseStatus === 429) return createHttpError(429, 'GitHub rate limit reached. Please wait.');
...
export async function githubRequest(path, options = {}) {
  ...
  try {
    ...
    if (error.name === 'AbortError') throw createHttpError(504, 'GitHub request timed out.');
...
export async function githubGetJson(requestPath) {
  return githubRequest(requestPath);
}
...
export async function commitFile(filePath, content, message, sha) {
  const body = {
    message,
